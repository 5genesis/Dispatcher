...
class NbiUtil():
    ...
    def modify_vnfd_package(self, file, vnfd_id):
        """
        Onboards a VNFD package to modify an existing one

        Parameters
        ----------
        - file: Modififed VNF package to replace the old one
        - vnfd_id: ID of the VNF package
        ...
        headers["Content-type"] = "application/gzip"
        try:
            r = requests.put(vnfd_url, params=None, verify=False, stream=True, data=data, headers=headers)
            return r.text, r.status_code
        except Exception as e:
